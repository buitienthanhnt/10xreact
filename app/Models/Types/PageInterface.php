...
interface PageInterface extends TimestampInterface
{
	...
	const FORM_FIELDS = [
		...
		self::WRITER => ['key' => self::WRITER, 'type' => FormInterface::TYPE_SELECT, 'label' => 'tac gia', 'model' => \App\Models\Page::class]
	];

	/**
	 * khai báo danh sách các thuộc tính được gán hàng loạt.
	 */
	const FILLED_FILEDS = [self::TITLE, self::ACTIVE, self::ALIAS, self::IMAGE_PATH, self::DESCRIPTION, self::WRITER];
